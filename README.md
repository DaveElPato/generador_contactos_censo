...
## Scripts para automatizar los censos de DAT

## Existen dos scripts para delegados: census_inicial y census_post_elecciones 

El censo inicial (census_inicial) está pensado para usarse una vez que hayan salido todos los delegados de grupo/clase, que se hayan registrado correctamente en el Qform y estén reflejados en el Excel del censo.

### Pasos para ejecutar el script
- Descargar el script
- Crear una carpeta nueva y mover el script a dicha carpeta
- Mover el csv a usar a la misma carpeta (normalmente venrá de un Qform o exportado de una tabla excel)
- Editar el archivo para actualizar la fecha de la etiqueta "[DAT XX/XX+1]" al año actual (u otra etiqueta que se vaya a usar, en el caso de scripts como el de FDC)
- Verificar que los nombres de las columnas en el csv concuerdan con los parámetros del script (si se usa el mismo formato que el censo 24/25, deberían coincidir de antemano)
...
El censo post elecciones (census_post_elecciones) está pensado para usarse una vez se hayan llevado a cabo las elecciones de delegado de curso, titulación, consejos, JE, Claustro... entre otros. Antes de usarse, se deben borrar los contactos que se agregaron cuando se hizo el censo inicial. Para ello, seleccionar la etiqueta [DAT XX/XX+1], y después "Eliminar todos los contactos y esta etiqueta".
...
Adicionalmente, pueden existir más scripts para propósitos similares, como FDC. El modo de empleo es el mismo.
...
The initial census (census_inicial) should be used when all the group/class delegates have been elected, succesfully registered in the Qform and reflected in the census Excel.

### Steps for executing the scripts
- Download the script
- Put it in a separate new folder
- Add there the csv from the census you want to manage (usually coming from a Qform or an excel table)
- Edit the file to update the date of the tag "[DAT XX/XX+1]" to reflect the current year (or other tags if using other scripts, such as FDC)
- Check if the column names in the csv match the parameters in the script (if using the same format for the census as the 24/25 one, they should match beforehand)
...
The post elections census (census_post_elecciones) is meant to be used when the elections for year delegate, course delegate, department councils, JE, Cloister... and more have been celebrated. Before using, the contacts created when the script was last used should be deleted. To do this, select the [DAT XX/XX+1] tag, and then "Delete all contacts and delete this label".
...
Additionally, it is possible that more scripts for similar purposes, such as FDC, can exist. The steps to use them are the same.
